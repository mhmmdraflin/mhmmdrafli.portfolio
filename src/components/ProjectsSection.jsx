...
React.useEffect(() => {
    // Simple auto-play loop for mobile aesthetics
...
const getLeftPhoneClasses = () => {
    ...
    // Mobile "Active" State (simulates hover)
    const mobileActive = isMobileExpanded ? "-translate-x-24 rotate-y-[-15deg] scale-90" : "";
    // Desktop Hover (Native)
    const desktopInactive = "group-hover/phones:-translate-x-24"; // Fallback if needed, but we replace with mobileActive logic for consistency

    return `${base} ${desktopHover} ${mobileActive}`;
};
